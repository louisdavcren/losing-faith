...
sailors = [("Alice", [1, 2, 1, 1, 1,]), ("Bob", [2, 4, 1, 1, 2, 5]), ("Clare", [2, 3, 2, 2, 4, 2]), ("Dennis", [5, 4, 4, 4, 3, 4]), ("Eva", [4, 5, 3, 5, 5, 3])]


        # QUESTION 1.A
def series_score(): # person, worst=1
    sailors[1].remove(max(sailors[1])) # Remove the largest(worst) score

    series_score = sum(sailors[1]) # Add the scores together

    print(series_score) # Display the score

        # QUESTION 1.B
def sort_series():
    print(sorted(sailors, key=lambda x: x[0]))
